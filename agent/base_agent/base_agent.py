...
try:  # langchain <=0.1 style
    ...
    try:
        ...
        try:
            from langchain_core.callbacks.stdout import (
                StdOutCallbackHandler as _ConsoleHandler,
            )  # type: ignore
        except Exception:
...
class BaseAgent:
    ...
    async def run_trading_session(self, today_date: str) -> None:
        ...
        if self.verbose and _ConsoleHandler is not None:
            ...
            print(
                "⚠️ Verbose requested but no StdOut/Console callback handler found in current LangChain version."
            )
